...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//#define PushRod  200 //即推杆变量 m_nDist[2]，待修改 //ADC功能已实现，不需要使用PushRod宏定义
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//
void Push_Wheel(void);
void Draw_Wheel(void);
void Stop_Wheel(void);
//
void Init_Execute_Data(void);
void Move_Wheel(u8_t m_bDir);
void Adjust_Deflect(void);
void Adjust_Parallel(void);
//
void Control_Mid_Relay(u8_t on);
void Control_Error_Relay(u8_t on);
void Control_ErrLed_Relay(u8_t on);
//
void Init_IoPort(void);			//初始化I/O口
//
void Wheel_Pos_Control(void);//推杆位置检测
//
void Vehicle_State_Detect(void);//模式、方向、速度检测
